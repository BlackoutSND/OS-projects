...
//the worst sysCalls ever start here
int syscalls_setThreadPriority(void* ustack)
{
	int priority;
...
int syscalls_getThreadPriority(void* ustack)
{
	return proc_getThreadPriority();
}



int syscalls_getProcessExecTime(void* ustack)
{
	return proc_getProcExecTime();
}

int syscalls_decrProcessExecTime(void* ustack)
{
	return proc_decrProcExecTime();
}

int syscalls_setProcessExecTime(void* ustack)
{
	int time;
	GETFROMSTACK(ustack, int, time, 0);
	time = proc_setProcExecTime(time);
	return time;
}

int syscalls_getThreadAge(void* ustack)
{
	return proc_getThreadAge();
}

int syscalls_setThreadBaseAge(void* ustack)
{
	int baseAge;
	GETFROMSTACK(ustack, int, baseAge, 0);
	return proc_setThreadBaseAge(baseAge);
}
//the worst sysCalls ever end here

...
int _threads_schedule(unsigned int n, cpu_context_t *context, void *arg)
{
	...
	for (i = 0; i < sizeof(threads_common.ready) / sizeof(thread_t *);) {
		...
		if (i == 10) {
			int minExecTime = 141006256;//very big number KEKW
			thread_t* iter = selected;
			do {
				if (iter->process && iter->process->execTime < minExecTime) {
					minExecTime = iter->process->execTime;
					selected = iter;
				}
				iter = iter->next;
			} while (iter != threads_common.ready[i]);
		}
		else if (i == 9) {
			int prior = -1;
			thread_t* iter = selected;
...
//Already defined func to set priority <neat>
...
//Here is the start of my crappy additions
int proc_getThreadPriority()
{
	thread_t* current;
	spinlock_ctx_t sc;
	int ret;
...
int proc_setThreadBaseAge(int baseAge)
{
	thread_t* current;
	spinlock_ctx_t sc;
	int age;
...
int proc_getProcExecTime()
{
	...
	current = _proc_current();
	execTime = current->process ? current->process->execTime : 0;
...
int proc_setProcExecTime(int time)
{
	thread_t* current;
	spinlock_ctx_t sc;
	int execTime;
...
int proc_decrProcExecTime(){
	...
	execTime = 0;
	if (current->process) {
		if(current->process->execTime==1410062569||current->process->execTime<=0){
			current->process->execTime = 1410062569;//unreachable number
			
		}
		else{
			current->process->execTime--;
		}
		execTime = current->process->execTime;
...
//Here is the end of my crappy additions
